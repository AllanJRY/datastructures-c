...
void linked_list_print(Linked_List_Node* linked_list_head) {
    Linked_List_Node* curr_node = linked_list_head;
    for (;curr_node != NULL;) {
...
bool linked_list_lookup(Linked_List_Node* linked_list_head, int needle_val, size_t* found_idx) {
    assert(linked_list_head != NULL && "Linked List head is NULL.");
...
int main(void) {
    ...
    printf(" (%llu nodes)\n", linked_list_len(linked_list_head));
    ...
    size_t found_idx;
    bool found = linked_list_lookup(linked_list_head, 4, &found_idx);
    if (found) {
        printf("Lookup value `4` found at idx: %llu\n", found_idx);

        int removed_val;
        bool removed = linked_list_remove(linked_list_head, found_idx, &removed_val);
        if(removed) {
